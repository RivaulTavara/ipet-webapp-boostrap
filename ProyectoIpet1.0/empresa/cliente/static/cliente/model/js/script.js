...
$(function() {
    $(document).delegate('.price', 'click', async function() {
        if ($(this).hasClass('converted')) {
            ...
            // Si el precio no ha sido convertido, guarda el precio original
            $(this).data('originalPrice', $(this).find('a').text());

            // Obtén el texto del elemento <a>, elimina el símbolo del dólar y el punto
            ...
            try {
                // Realiza la solicitud a la API y espera a que se complete
                const response = await $.getJSON('https://mindicador.cl/api');

                // Convierte el precio a dólares
